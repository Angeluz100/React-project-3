const Card = (props) => {
  ...
  async function handleEdit(placeId) {
    const {place, comments, _id} = props.state.places.find(place => place._id === placeId);
    props.setState(prevState => ({
      ...prevState,
      newPlace: {
        place,
    ...
    props.state.places.map((s, idx) => (
        <div className="card">
          <article key={idx}>
            ...
            <div>
          { !props.state.editMode && <div className="edit"onClick={() => handleEdit(s._id)}>{"✒️"}</div>}
            <div className="delete"onClick={() => handleDelete(s._id)}>{"🗑"}</div>
            </div>
          </article>
        </div>
      ))
